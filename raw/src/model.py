...
class PathCon(nn.Module):
    def __init__(self, args, n_relations, n_attributes, params_for_neighbors, params_for_paths):
        super(PathCon, self).__init__()
        self._parse_args(args, n_relations, n_attributes, params_for_neighbors, params_for_paths)
        self._build_model()

    def _parse_args(self, args, n_relations, n_attributes, params_for_neighbors, params_for_paths):
    ...
    def _build_relation_and_attribute_feature(self):
        if self.feature_type == 'id':
            ...
            self.attribute_dim = self.n_attributes
            self.aggregator_input_dim = self.n_relations + self.n_attributes*2
        ...
        self.attribute_features = torch.cat([self.attribute_features,
                                             torch.zeros([1, self.attribute_dim]).cuda() if self.use_gpu \
                                                 else torch.zeros([1, self.attribute_dim])], dim=0)
    ...
    def _get_neighbors_and_masks(self, relations, entity_pairs, train_edges):
        ...
        for i in range(self.context_hops):
            ...
            neighbor_edge_2_entities = torch.where(neighbor_edge_2_entities > len(self.entity2attributes)-1,
                                                   torch.tensor(len(self.entity2attributes) - 1).cuda(), neighbor_edge_2_entities)
            neighbor_edge_2_entities = torch.where(neighbor_edge_2_entities < 0,
                                                   torch.tensor(len(self.entity2attributes) - 1).cuda(), neighbor_edge_2_entities)

            neighbor_edge_2_entity_attributes = torch.index_select(self.entity2attributes, 0, neighbor_edge_2_entities.view(-1)).view([self.batch_size, -1, 2])
    ...
    def _aggregate_neighbors(self, attribute_list, edge_list, mask_list):
        ...
        attribute_vectors = [
            torch.index_select(self.attribute_features, 0, attribute_list[0].view(-1)).view(self.batch_size, 2, -1)]
