...
class PathCon(nn.Module):
    def __init__(self, args, n_relations, n_attributes, params_for_neighbors, params_for_paths, attribute_vectors):
        super(PathCon, self).__init__()
        self._parse_args(args, n_relations, n_attributes, params_for_neighbors, params_for_paths, attribute_vectors)
        self._build_model()

    def _parse_args(self, args, n_relations, n_attributes, params_for_neighbors, params_for_paths, attributes_vectors):
        self.n_relations = n_relations
        self.n_attributes = n_attributes
        self.embedded_attribute_vectors = torch.FloatTensor(attributes_vectors).cuda() if args.cuda \
                else torch.FloatTensor(attributes_vectors)

        self.each_axis_dim_for_attribute_embedding = args.each_axis_dim_for_attribute_embedding
        self.axis_for_attribute_embedding = args.axis_for_attribute_embedding
        self.total_dim_for_attribute_embedding = args.dim_for_attribute_embedding
        ...
        if self.use_context:
            ...
            self.entity2attributes = torch.LongTensor(params_for_neighbors[3]).cuda() if args.cuda \
                else torch.LongTensor(params_for_neighbors[3])
    ...
    def _build_model(self):
        ...
        if self.use_context or (self.use_path and self.path_type == 'rnn'):
            self._build_relation_and_attribute_feature()
    ...
    def _call_model(self):
        ...
        if self.use_context:
            attribute_list, edge_list, mask_list = self._get_neighbors_and_masks(self.labels, self.entity_pairs,
                                                                                 self.train_edges)

            self.aggregated_neighbors = self._aggregate_neighbors(attribute_list, edge_list, mask_list)
    ...
    def _build_relation_and_attribute_feature(self):
        if self.feature_type == 'id':
            self.relation_dim = self.n_relations
            self.attribute_dim = self.total_dim_for_attribute_embedding
            self.aggregator_input_dim = self.n_relations + self.total_dim_for_attribute_embedding * 2
            self.relation_features = torch.eye(self.n_relations).cuda() if self.use_gpu \
                else torch.eye(self.n_relations)
            self.attribute_features = torch.eye(self.n_attributes).cuda() if self.use_gpu \
                else torch.eye(self.n_attributes)
        ...
        self.attribute_features = torch.cat([self.attribute_features,
                                             torch.zeros([1, self.n_attributes]).cuda() if self.use_gpu \
                                                 else torch.zeros([1, self.n_attributes])], dim=0)

    # attribute_list, edge_list, mask_list = self._get_neighbors_and_masks(self.labels, self.entity_pairs, self.train_edges)
    def _get_neighbors_and_masks(self, relations, entity_pairs, train_edges):
        edges_list = [relations]
        entity_attribute_list = [torch.index_select(self.entity2attributes, 0, entity_pairs.view(-1)).view(-1, 2)]
        masks = []
        ...
        for i in range(self.context_hops):
            if i == 0:
                ...
                neighbor_entity_index = edges_list[-1].view(-1)
                neighbor_entities = torch.index_select(self.edge2entities, 0,
                                                       neighbor_entity_index).view([self.batch_size, -1])
            ...
            neighbor_edge_2_entities = torch.index_select(self.edge2entities, 0, neighbor_edges.view(-1)).view(-1)

            neighbor_edge_2_entities = torch.where(neighbor_edge_2_entities > len(self.entity2attributes) - 1,
                                                   torch.tensor(len(self.entity2attributes) - 1).cuda() if self.use_gpu \
                                                       else torch.tensor(len(self.entity2attributes) - 1),
                                                   neighbor_edge_2_entities)
            neighbor_edge_2_entities = torch.where(neighbor_edge_2_entities < 0,
                                                   torch.tensor(len(self.entity2attributes) - 1).cuda() if self.use_gpu \
                                                       else torch.tensor(len(self.entity2attributes) - 1),
                                                   neighbor_edge_2_entities)

            neighbor_edge_2_entity_attributes = torch.index_select(self.entity2attributes, 0,
                                                                   neighbor_edge_2_entities.view(-1)).view(
                [self.batch_size, -1, 2])
            entity_attribute_list.append(neighbor_edge_2_entity_attributes)

            mask = neighbor_edges - train_edges  # [batch_size, -1]
            mask = (mask != 0).float()
            masks.append(mask)
        return entity_attribute_list, edges_list, masks
    ...
    def _get_neighbor_aggregators(self):
        ...
        if self.context_hops == 1:
            aggregators.append(self.neighbor_agg(batch_size=self.batch_size,
                                                 input_dim=self.aggregator_input_dim,
                                                 output_dim=self.aggregator_input_dim,
                                                 self_included=False))
        else:
            # the first layer
            aggregators.append(self.neighbor_agg(batch_size=self.batch_size,
    ...
    def _aggregate_neighbors(self, attribute_list, edge_list, mask_list):
        edge_vectors = [torch.index_select(self.relation_features, 0, edge_list[0])]
        attribute_vectors = [
            torch.index_select(self.embedded_attribute_vectors, 0, attribute_list[0].view(-1)).view(self.batch_size, 2, -1)]

        attribute_head_vectors1 = attribute_vectors[0].chunk(2, dim=1)[0].squeeze(dim=1)
        attribute_tail_vectors1 = attribute_vectors[0].chunk(2, dim=1)[1].squeeze(dim=1)

        edge_and_attribute_vectors = [
            torch.cat((edge_vectors[0], attribute_head_vectors1, attribute_tail_vectors1), 1)]
        ...
        for i in range(1, len(edge_list)):
            ...
            attributes = attribute_list[i].view(list(attribute_list[i].shape) + [-1])

            current_edge_vector = torch.index_select(self.relation_features, 0,
                                                     relations.view(-1)).view(list(relations.shape) + [-1])

            # current_attribute_vector = torch.index_select(self.attribute_features, 0,
            #                                               attributes.view(-1)).view(list(attributes.shape) + [-1])

            current_attribute_vector = torch.index_select(self.embedded_attribute_vectors, 0,
                                                          attributes.view(-1)).view(list(attributes.shape) + [-1])

            current_head_attribute_vector = current_attribute_vector.chunk(2, dim=2)[0].squeeze(2)
            current_tail_attribute_vector = current_attribute_vector.chunk(2, dim=2)[1].squeeze(2)

            current_edge_and_attribute_vector = torch.cat((current_edge_vector,
                                                           current_head_attribute_vector,
                                                           current_tail_attribute_vector), -1)

            edge_vectors.append(current_edge_vector)
            attribute_vectors.append(current_attribute_vector)
            edge_and_attribute_vectors.append(current_edge_and_attribute_vector)
        ...
        for i in range(self.context_hops):
            ...
            edge_and_attribute_vectors_next_iter = []
            ...
            for hop in range(self.context_hops - i):
                ...
                vector = aggregator(self_vectors=edge_and_attribute_vectors[hop],
                                    neighbor_vectors=edge_and_attribute_vectors[hop + 1].view(neighbors_shape),
                                    masks=mask_list[hop].view(masks_shape))
                edge_and_attribute_vectors_next_iter.append(vector)
            edge_and_attribute_vectors = edge_and_attribute_vectors_next_iter

        # edge_vectors[0]: [self.batch_size, 1, self.n_relations]
        res = edge_and_attribute_vectors[0].view([self.batch_size, self.aggregator_input_dim])
        res = res[:, 0:self.n_relations]
        return res
    ...
    def _rnn(self, path_ids):
        ...
        paths = torch.index_select(self.id2path, 0,
                                   path_ids.view(-1)).view(
            list(path_ids.shape) + [-1])  # [batch_size * path_samples, max_path_len]
