...
def read_attributes(file_name):
    ...
    attribute_list = []
    for line in file:
        index, attribute = line.strip().split('\t')
        if attribute not in attribute_list:
            attribute_list.append(attribute)
        d.append(attribute_list.index(attribute))
    file.close()

    return d


def read_attribute_vectors(file_name):
    d = []
    file = open(file_name)
    # attribute_vectors_list = []
    for line in file:
        index, attribute = line.strip().split('\t')
        attribute_vector = eval(attribute)
        if attribute_vector not in d:
            # attribute_vectors_list.append(attribute_vector)
            d.append(attribute_vector)
    empty_attribute = [0] * dim_for_attribute_embedding
    d.append(empty_attribute)
    file.close()
...
def build_kg(train_data):
    for edge_idx, triplet in enumerate(train_data):
        ...
        if args.use_context:
            ...
            # head_attribute = attribute_dict[head_idx]
            # tail_attribute = attribute_dict[tail_idx]

            edge2entities.append([head_idx, tail_idx])
            # entity2attributes.append([head_attribute, tail_attribute])
    ...
    if args.use_context:
        ...
        entity2attributes.append([null_entity, null_entity])
...
def load_data(model_args):
    global args, entity_dict, relation_dict, attribute_dict, n_attributes, dim_for_attribute_embedding
    args = model_args
    directory = '../data/' + args.dataset + '/'
    print('Embedding model, the current dataset is：', args.dataset)

    entity_dict = read_entities(directory + 'entities.dict')
    relation_dict = read_relations(directory + 'relations.dict')
    dim_for_attribute_embedding = args.dim_for_attribute_embedding
    attribute_dict = read_attributes(directory + 'attributes.dict')
    attribute_vectors = read_attribute_vectors(directory + 'attribute_vectors.txt')
    entity2attributes = attribute_dict
    n_attributes = len(np.unique(attribute_dict))
    ...
    if args.use_context:
        neighbor_params = [np.array(entity2edges), np.array(edge2entities),
                           np.array(edge2relation), np.array(entity2attributes)]
    else:
    ...
    return triplets, paths, len(relation_dict), n_attributes, neighbor_params, path_params, attribute_vectors
